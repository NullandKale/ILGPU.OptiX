...
namespace Sample21
{
    ...
    public static class MaterialShading
    {
        ...
        // spanning diffuse through perfect-mirror response.
        ...
        internal unsafe static void ShadeSurface(LaunchParams launchParams, MaterialSbtData mat, Vec3 baseColor, Vec3 surfPos, Vec3 shadingNormal, Vec3 outwardNormal, Vec3 rayDir, uint rngStateIn, float bsdfPdfIn, float lightPickAreaPdf, Vec2 uv)
        {
            ...
            // albedo for a flat ambient term to reflect into (a real irradiance
            // environment would still light metals via the specular lobe; this flat
            // constant-ambient term is a fallback for scenes with no HDRI - see
            // SceneData.EnvMapPath. HDRI-lit scenes zero AmbientIntensity and let the
            // specular lobe sample the environment map directly instead).
            ...
            if (isDelta)
            {
                ...
                Payloads.SetContinuePayload(pixelColor, Payloads.BOUNCE_CONTINUE_MIRROR, newOrigin, reflectDir, mirrorTint, shadingNormal, baseColor, rng.State, Payloads.DeltaOrPrimarySentinel, surfPos, roughness, metallic);
                return;
            }
            ...
            if (NdotSampled <= 0f)
            {
                Payloads.SetTerminalPayload(pixelColor, shadingNormal, baseColor, surfPos, roughness, metallic);
                return;
            }
            ...
            Payloads.SetContinuePayload(pixelColor, bounceFlag, newOrigin, sampledDir, tint, shadingNormal, baseColor, rng.State, combinedPdf, surfPos, roughness, metallic);
        }
        ...
        // reduces to a perfect-specular path as a degenerate case (same delta-lobe
        ...
        // The VNDF-sampled lobe's unbiased throughput weight simplifies to the Smith
        // masking ratio G2/G1(v) (Heitz's well-known VNDF-sampling simplification) -
        // identical form for both the reflect and refract branches, differing only in
        // which direction NdotL is measured toward.
        internal unsafe static void ShadeDielectric(LaunchParams launchParams, MaterialSbtData* sbtData, Vec3 rayDir, Vec3 outwardNormal, bool frontFace, Vec3 surfPos, uint rngStateIn)
        {
            ...
            Payloads.SetContinuePayload(sbtData->Emission, Payloads.BOUNCE_CONTINUE_DIELECTRIC, newOrigin, newDir, tint, outwardNormal, tint, rng.State, Payloads.DeltaOrPrimarySentinel, surfPos, roughness, sbtData->Metallic);
        }
        ...
        // BaseColorTexture == 0 means no texture. UV is only ever non-zero for triangle
        ...
        internal unsafe static Vec3 SampleAlbedo(MaterialSbtData* sbtData, Vec3 surfPos, Vec2 uv)
        {
            ...
            // OETF). Applied only to the texture sample, not the scalar BaseColor
