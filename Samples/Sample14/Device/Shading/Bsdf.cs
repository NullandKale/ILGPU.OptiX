...
namespace Sample14
{
    ...
    //
    // Reflection only - dielectric transmission (glass) is a separate rough BTDF path
    // (Walter et al. 2007, MaterialShading.ShadeDielectric). Diffuse + specular are
    // combined via single-sample stochastic lobe selection (ShadeSurface picks one
    // lobe to extend the path with, weighted by the *combined* pdf of both lobes at
    // the sampled direction - the standard unbiased multi-lobe MC estimator), not
    // per-lobe MIS.
    internal static class Bsdf
    {
        ...
        // defaults to 0) rendering as an exact mirror.
        ...
        // used by MaterialShading.ShadeDielectric.
        ...
        // with the shading normal. Exposed separately from SampleSpecularDirection so
        // the rough dielectric BTDF (MaterialShading.ShadeDielectric) can sample the
        // same microfacet distribution and then reflect *or* refract about it, instead
        // of only ever reflecting.
