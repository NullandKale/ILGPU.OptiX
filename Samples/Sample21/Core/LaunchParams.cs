...
namespace Sample21
{
    ...
    public struct LaunchParams
    {
        ...
        // Unified bounce budget shared by every material kind's raygen bounce loop;
        // Russian roulette (RaygenProgram.cs) is what actually terminates most paths
        // early in practice, this is just the hard ceiling.
        ...
        // NRC - written by RaygenProgram.cs's NRC side channel (see its own doc
        // comment), consumed by the self-training-target-resolution, training, and
        // composite stages (Rendering/SampleRenderer.cs). Invalid/zero-length
        // (OptixDeviceView<T>.IsValid == false) until NRC buffers are allocated -
        // RaygenProgram.cs guards every write on NrcEvalRecords.IsValid, so a
        // scene/frame with NRC disabled pays nothing extra.
        public OptixDeviceView<EvalRecord> NrcEvalRecords;
        public OptixDeviceView<TrainRecord> NrcTrainRecords;
        // One atomic counter per training batch (NrcConstants.TrainBatchesPerFrame
        // elements) - each TrainRecord is randomly assigned to one of the 4 disjoint
        // per-frame batches at write time (RaygenProgram.cs), matching VkNRC's own
        // `batch = clamp(uint(RNGNext()*4), 0, 3)` scheme rather than one pool
        // reprocessed 4 times.
        public OptixDeviceView<int> NrcTrainCounter;

        // Runtime-tunable (UI/UiPanel.cs's NRC section), so the user can adjust them
        // live instead of recompiling per experiment.
        //
        // NrcEnabled gates ALL adaptive-handoff/eval/train bookkeeping below in
        // RaygenProgram.cs - when 0, the bounce loop runs exactly as it would with NRC
        // never having existed (full path trace, no truncation, no record writes).
        public int NrcEnabled;
        // VkNRC's "C" footprint constant (NrcConstants.FootprintConstant's live-tunable
        // mirror) - see RaygenProgram.cs's adaptive handoff test.
        public float NrcFootprintConstant;
        public float NrcTrainProbability;
        // How many of the NrcConstants.TrainBatchesPerFrame batch slices raygen
        // actually distributes this frame's train records across (1..TrainBatchesPerFrame)
        // - the performance knob SampleRenderer.NrcTrainBatchesPerFrame. Records
        // concentrate into the slices the training loop will actually consume, rather
        // than landing in batches nothing trains on; the buffers/counters stay sized
        // for the full TrainBatchesPerFrame worst case regardless.
        public int NrcTrainBatchCount;

        // NRC scene-position normalization. VkNRC bakes a [-1,1] normalization into
        // every model's VERTICES at load time (example/VkNRC/src/Scene.cpp's obj_load:
        // vertex = (vertex - center) * 2/maxExtent) - that is the hidden contract behind
        // InputEncoding.Encode's frequency bands (triangle wave of period 2, up to
        // 2048x). This port keeps raw world-space geometry, so RaygenProgram.cs applies
        // the equivalent remap to positions at NRC record-write time instead - without
        // it, position features at raw OBJ scale wrap the frequency bands hundreds of
        // times over and exceed fp32 mod-2 precision, degenerating into a spatial hash.
        // NrcSceneInvExtent = 2 / largest scene-AABB extent; both set per scene by
        // SampleRenderer.SwitchToScene.
        public Vec3 NrcSceneCenter;
        public float NrcSceneInvExtent;
    }
}
