...
namespace Sample21
{
    ...
    //
    // Reflection only - the dielectric transmission (glass) lobe is handled separately
    // by MaterialShading.ShadeDielectric's Fresnel-Schlick path (perfect-specular
    // unless TransmissionRoughness > 0, Walter et al. 2007 rough BTDF). Diffuse +
    // specular are combined via single-sample stochastic lobe selection
    // (ShadeSurface picks one lobe to extend the path with, weighted by the
    // *combined* pdf of both lobes at the sampled direction - the standard unbiased
    // multi-lobe MC estimator), not per-lobe MIS.
    internal static class Bsdf
    {
        ...
        // finite BRDF value anyway. This is what keeps MaterialPresets.Mirror()
        // (Roughness defaults to 0) rendering as an exact mirror.
        ...
        // with the shading normal. Split out from SampleSpecularDirection so the rough
        // dielectric BTDF (MaterialShading.ShadeDielectric) can sample the same
        // microfacet distribution and then reflect *or* refract about it, instead of
        // only ever reflecting.
