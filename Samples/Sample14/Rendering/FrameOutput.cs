...
namespace Sample14
{
    ...
    public sealed class FrameOutput : IDisposable
    {
        ...
        // "current" and "previous" swap every frame (see SwapColorHistory). Blending by
        // reprojected world position (not pixel index) means this survives camera moves
        // without a hard reset (see SampleRenderer.cs's MaxHistoryFrames).
        ...
        public unsafe void DenoiseAndTonemap(bool denoiserOn, bool accumulate, int frameId, float exposureStops, int tonemapOperator, out double denoiseMs, out double tonemapMs)
        {
            ...
            if (denoiserOn && builtDenoiser != null)
            {
                ...
                // BlendFactor: 0 = fully denoised, 1 = unmodified raw input. FrameID
                // does not reset on camera move (RaygenProgram.cs's own reprojected
                // accumulation is the real convergence mechanism), so this settles
                // toward "mostly denoised" over the session rather than spiking back
                // to fully-raw on every camera move.
