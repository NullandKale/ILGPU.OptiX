...
namespace Sample21
{
    ...
    ///    HDR to LDR range
    /// 3. sRGB OETF (piecewise, not an approximation) - linear to display-referred
    ///    sRGB
    ...
    public static class TonemapKernel
    {
        ...
        // True piecewise sRGB OETF (IEC 61966-2-1), not a flat pow(x, 1/2.2)
        // approximation - the linear segment near black matters for shadow-detail
        // banding that a flat gamma curve doesn't reproduce exactly.
        ...
        // Tone mapping + row flip for display. exposureMultiplier is the already-
        // evaluated 2^exposureStops (photographic-stops convention, each +1 doubles
        // linear radiance) - computed ONCE on the host (FrameOutput.TonemapToDisplay)
        // rather than every one of width*height per-pixel threads redundantly calling
        // XMath.Pow for the same launch-uniform result. tonemapOperator selects
        // OperatorReinhard/OperatorAces (see SampleRenderer's own Exposure/
        // TonemapOperator properties and the UI panel's controls).
        public static void tonemapAndFlip(Index1D index, int width, int height, float exposureMultiplier, int tonemapOperator, ArrayView<Vec4> hdr, ArrayView<byte> dest)
        {
            int x = index % width;
