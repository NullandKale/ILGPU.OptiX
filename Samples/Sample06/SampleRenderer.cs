...
namespace Sample06
{
    ...
    public class SampleRenderer
    {
        ...
        public unsafe IntPtr buildAccel(TriangleMesh[] meshes)
        {
            IntPtr asHandle = IntPtr.Zero;
            int n = meshes.Length;

            // One vertex-buffer-pointer slot and one triangle-flags slot per mesh,
            // allocated once (not per loop iteration - stackalloc in a loop risks
            // unbounded stack growth).
            var vertexBufferPtrs = stackalloc IntPtr[n];
            var triangleFlagsPtrs = stackalloc uint[n];

            var buildInputs = new OptixBuildInput[n];
            for (int meshId = 0; meshId < n; meshId++)
            {
                ...
                vertexBufferPtrs[meshId] = mesh.d_vertexBuffer.NativePtr;
                ...
                triangleInput.TriangleArray.NumVerticies = (uint)mesh.vertexBuffer.Count;
                triangleInput.TriangleArray.VertexBuffers = new IntPtr(vertexBufferPtrs + meshId);
                ...
                triangleInput.TriangleArray.NumIndexTriplets = (uint)mesh.triangleIndexBuffer.Count;
                triangleInput.TriangleArray.IndexBuffer = mesh.d_triangleIndexBuffer.NativePtr;

                triangleInput.TriangleArray.Flags = triangleFlagsPtrs + meshId;
                triangleInput.TriangleArray.NumSbtRecords = 1;
                triangleInput.TriangleArray.SbtIndexOffsetBuffer = IntPtr.Zero;
                triangleInput.TriangleArray.SbtIndexOffsetSizeInBytes = 0;
                triangleInput.TriangleArray.SbtIndexOffsetStrideInBytes = 0;

                buildInputs[meshId] = triangleInput;
            }
            ...
            OptixAccelBufferSizes blasBufferSizes = deviceContext.AccelComputeMemoryUsage(accelOptions, buildInputs);
