...
namespace Sample21
{
    ...
    // calls, which pack this struct immediately after each record's header. Only the
    // radiance hitgroup record's data is ever read (the shadow ray's closest-hit is an
    // empty stub).
    //
    // A glTF-style metallic-roughness PBR material model.
    //
    // Shading dispatch (__closest__radiance): Transmission > 0 -> ShadeDielectric's
    // Fresnel-Schlick glass (perfect-specular unless TransmissionRoughness > 0); else ->
    // ShadeSurface's unified GGX metallic-roughness BSDF (Bsdf.cs), which internally
    // treats Roughness below Bsdf.DeltaRoughnessThreshold as a perfect mirror and
    // otherwise continuously blends Lambertian diffuse against Fresnel-tinted GGX
    // specular by Metallic. MaterialKind is an orthogonal second axis (Solid vs. Checker
    // base-color sampling only - checkered surfaces use whichever BSDF branch their
    // Roughness/Metallic pick).
    public struct MaterialSbtData
    {
        ...
        // 0 = opaque, 1 = fully transmissive (glass); read as a > 0 threshold by the
        // dispatch above.
        ...
        // (0) is a deliberate, desired degenerate case: it produces perfect-specular
        // glass unless a scene explicitly opts into a rough/frosted look.
        public float TransmissionRoughness;

        // Texture handles, 0 = none. OrmTexture packs occlusion.r/roughness.g/metallic.b
        // (MaterialShading.cs's ApplyNormalMap/ShadeSurface).
        ...
        // default, same trap as the base Roughness field). Any material that sets
        // NormalTexture MUST also explicitly set NormalStrength (1f unless a
        // deliberately subtle effect is wanted), or the texture will be sampled and
        // then silently multiplied away to nothing by ApplyNormalMap.
        public float NormalStrength;

        // Blend weight between the scalar BaseColor and BaseColorTexture's sample;
        // reused as the ORM/normal texture blend weight too.
        ...
        // Read by __anyhit__radiance/__anyhit__shadow, which OptixIgnoreIntersection.
        // Ignore() any triangle hit whose sampled alpha falls below this value.
        public float AlphaCutoff;

        // Index into the emissive-triangle light list for materials with nonzero
        // Emission, or -1 if this material has no light-list entry. Set by
