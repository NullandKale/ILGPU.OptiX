...
namespace Sample21
{
    ...
    // A minimal, from-scratch Wavefront OBJ/MTL loader - one merged vertex/index buffer
    // for the whole model, per-material data looked up via optixGetSbtDataPointer
    // against one hitgroup record per material (see SampleRenderer.cs), same
    // convention as every other scene in this sample.
    public class OBJModel
    {
        ...
        public static OBJModel Load(string objPath)
        {
            ...
            foreach (var rawLine in File.ReadLines(objPath))
            {
                ...
                switch (tokens[0])
                {
                    ...
                    case "vt":
                        ...
                        // convention), not the raw OBJ-spec bottom-left origin. No other
                        // place in the pipeline touches V (SampleAlbedo/SampleAlpha/
                        // SampleOrm/ApplyNormalMap all use the interpolated uv.y as-is -
                        // see MaterialShading.cs). If a *different* OBJ asset is ever
                        // added that genuinely does follow the spec's bottom-left
                        // convention, it will need its own per-asset flip instead of a
                        // global one here.
            ...
            Console.WriteLine($"[OBJ] Loaded {Path.GetFileName(objPath)}: {model.Vertices.Length} vertices, {model.Indices.Length} triangles, {model.Materials.Length} materials");
