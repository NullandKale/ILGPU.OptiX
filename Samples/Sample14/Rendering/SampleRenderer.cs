...
namespace Sample14
{
    ...
    public class SampleRenderer
    {
        ...
        // many frames, whether the camera is static OR moving, since it blends by
        // reprojected world position rather than pixel index. Toggling this off (or an
        ...
        // A long-lived history survives many small per-frame reprojection resamples
        // during gradual camera motion (each
        ...
        // 0 or negative disables decay entirely (freezes at the cap).
        ...
        public void setCamera(Camera camera)
        {
            lock (gpuLock)
            {
                ...
                // FrameID does not reset on camera move - it's a simple "frames since
                // scene load" counter (RaygenProgram.cs's own per-pixel reprojected
                // accumulation is what actually converges the image, see
                // MaxHistoryFrames). Keeping FrameID monotonic also matters because
                // RaygenProgram.cs seeds its per-pixel RNG from FrameID; resetting it
                // to 0 on every move frame would make continuous camera motion reuse
                // the exact same jittered sample pattern every frame.
            }
        }
        ...
        public void render()
        {
            lock (gpuLock)
            {
                ...
                // straight to FrameOutput.ResolveTaa below, not through LaunchParams - only
