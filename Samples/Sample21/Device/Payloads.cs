...
namespace Sample21
{
    ...
    public static class Payloads
    {
        ...
        internal struct RadiancePayload
        {
            ...
            // NRC input-encoding features - Roughness/Metallic at the CURRENT hit,
            // read back every bounce (not just bounce 0 like Normal/Albedo's AOV-only
            // use) since
            // Device/Network/InputEncoding.cs's per-hit encoding needs them at whichever
            // bounce a training suffix or NRC hand-off happens to occur at. Exactly at
            // OptixTrace.MaxPayloadCount (26) - see that constant's own doc comment for
            // why this is a hard ceiling, not just a convention.
            public float RoughnessOut;                        // 24
            public float MetallicOut;                         // 25
        }
        ...
        // bounce==0 Trace() call (see __raygen__renderFrame) - an AOV guide-buffer
        // convention of reflecting only the primary-ray hit, not anything accumulated
        // across bounces - but every shading branch still populates them on every
        // call, since raygen has no way to know in advance which bounce a given
        // closest-hit invocation corresponds to.
        internal static void SetTerminalPayload(Vec3 contribution, Vec3 normal, Vec3 albedo, Vec3 hitPos, float roughness = 0f, float metallic = 0f)
        {
            ...
            // RaygenProgram.cs's __raygen__renderFrame), so writing 0 is not observable.
        ...
        internal static void SetContinuePayload(Vec3 contribution, uint flag, Vec3 newOrigin, Vec3 newDir, Vec3 tint, Vec3 normal, Vec3 albedo, uint rngState, float bsdfPdf, Vec3 hitPos, float roughness = 0f, float metallic = 0f)
        {
            var payload = new RadiancePayload
            {
                RadianceX = contribution.x,
