...
namespace Sample21
{
    ...
    /// frames have been accumulated. RaygenProgram's own depth-based disocclusion
    /// check (trustHistory) is the only defense against stale/incorrect history.
    /// </summary>
    public static class TaaResolveKernel
    {
        ...
        // historyDecayFactor is the already-evaluated per-frame decay multiplier
        // (0.5^(deltaTimeSeconds/halfLifeSeconds), or 1 when decay is disabled) -
        // computed ONCE on the host (FrameOutput.ResolveTaa) since deltaTimeSeconds/
        // historyDecayHalfLifeSeconds are both launch-uniform, not per-pixel; every one
        // of width*height threads redundantly calling XMath.Pow for the identical
        // result would otherwise cost a transcendental call per pixel per frame for no
        // reason.
            ...
            ArrayView<float> outAccumCount)
        {
            ...
            if (trustHistory)
            {
                ...
                // weight once it hits maxHistoryFrames. historyDecayFactor is 1 when
                // decay is disabled (a no-op multiply, cheaper than branching around it).
                prevCount *= historyDecayFactor;
