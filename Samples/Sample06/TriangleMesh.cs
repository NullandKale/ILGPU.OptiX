...
namespace Sample06
{
    // Unlike Sample04/05's single merged TriangleMesh, this represents ONE independent
    // object - Sample06's SampleRenderer holds a List<TriangleMesh>, each becoming its
    // own OptixBuildInput within a single acceleration structure (mirroring
    // optix7course's example06_multipleObjects, which builds one GAS from multiple
    // per-object build inputs rather than one merged mesh).
    public class TriangleMesh
    {
        ...
        public TriangleMesh(Accelerator accelerator, Vec3 color)
        {
